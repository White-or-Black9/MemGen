#!/usr/bin/env bash
# Full EventQA top-k=1 sensitivity under formal frozen P7 settings.
...
for seed in "$@"; do
  CUDA_VISIBLE_DEVICES="$gpu_index" "$python_bin" \
    eval/exp/hyperparameter_sensitivity/eventqa_p7_topk1.py \
    --output-root "$run_root/seed${seed}" --requested-contexts 5 \
    --seed "$seed" --reseed-per-context --max-slots 16 \
    --retrieve-threshold 0.05 --update-threshold 0.10 \
