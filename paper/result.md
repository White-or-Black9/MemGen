...
controls. All three controls remain below the latent memory manager on both EM
and recall.
BM25 top-2 achieves recall of 0.226 but EM of only 0.030, while the 16-token
matched-budget condition reaches EM of 0.068. Together with the capacity-max
MemGen recent-text comparison in Table 1, these controls show that the observed gain is not reproduced by either small retrieved-text budgets or a recent text
suffix that nearly fills the model context.
...
**Table 2 | Comparison with explicit-memory controls.** All three textual
controls use five complete process-level passes with the fixed base seed;
...
Table 3 separates bank access, retrieved-memory consumption, and Weaver-mediated integration during query-time memory use. The no-query-retrieval ablation obtains an EM of $0.008$, recall of $0.178$, and $377.0$ format failures, showing that constructing a bank alone is insufficient to produce the full-method improvement.
...
**Table 3 | Query-time memory-use ablations.** Values are mean $\pm$
