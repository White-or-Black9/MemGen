...
for context_index in 3 4; do
  echo "MATCHED16_RECOVERY_START context=${context_index} timestamp=$(date --iso-8601=seconds)"
  ...
  echo "MATCHED16_RECOVERY_DONE context=${context_index} timestamp=$(date --iso-8601=seconds)"
done

echo "MATCHED16_RECOVERY_QUEUE_DONE timestamp=$(date --iso-8601=seconds)"
