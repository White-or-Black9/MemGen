#!/usr/bin/env bash
# One-process, five-context audited cost run for the recent-text MemGen baseline.
set -euo pipefail
...
run_id="${run_stamp}-eventqa-memgen-recent-window-continuous-controlled-cost"
output_root="outputs/mab/eventqa_memgen_recent_window_continuous_controlled_cost/${run_id}"
log_root="runtime_logs/eventqa_memgen_recent_window_continuous_controlled_cost/${run_id}"
...
CUDA_VISIBLE_DEVICES="$gpu_index" "$python_bin" scripts/eval/eventqa_memgen_recent_window_continuous_cost.py \
